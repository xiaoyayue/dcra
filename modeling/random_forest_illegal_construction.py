...
dat = pd.read_csv('../../Data/final_illegal_construction_inspections.csv')
...
rfmodel.fit(X_train_res, y_train_res)
cv10 = cross_val_score(rfmodel, X_train_res, y_train_res, cv=10)
...
plt.ylabel('Variable')
plt.title('Variable Importance Decision Tree Illegal Construction',dpi=300, bbox_inches = "tight")
plt.show()
fig2.savefig('Plots/Variable_Importance_IC.png')
...
plt.title('ROC Curve Random Forest Illegal')
plt.legend(loc = 'lower right')
plt.show()
fig.savefig('Plots/Random_Forest_ROC_Curve_Illegal_Construction.png',dpi=300, bbox_inches = "tight")


#getting a list of properties with a probability of being vacant>0.5
y_pred = rfmodel.predict(X)
y_pred = pd.DataFrame(y_pred)
y_pred['inspection_prob'] = y_pred[0]
...
pred_vac=pred_vac[['inspection_prob', 'SSL', 'LATITUDE','LONGITUDE', 'WARD']]
pred_vac=pred_vac.sort_values(by=['inspection_prob'], ascending=False)
pred_vac.head()

#save as csv to use for mapping
pred_vac.to_csv('../../Data/pred_ic_rf.csv')
...
#there are 10,602 false possitives
